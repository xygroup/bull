...
Queue.prototype.jobFromJSON = function(json, jobId) {
  return Job.fromJSON(this, json, jobId)
}
...
/**
  Returns a promise that resolves to the next job in queue.
*/
Queue.prototype.getNextJobNoWait = async function() {
  if (this.closing) {
    return null
  }

  if (this.drained) {
    let jobId = await this.client.rpoplpush(this.keys.wait, this.keys.active)
    return (jobId ? this.moveToActive(jobId) : null)
